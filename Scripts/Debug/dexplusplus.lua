			...
			Explorer.ShowRightClick = function(MousePos)
				...
				if presentClasses["LuaSourceContainer"] then
					context:AddRegistered(
						"VIEW_SCRIPT",
						not presentClasses.isViableDecompileScript or not env.isdecompile
					)
					context:AddRegistered(
						"DUMP_FUNCTIONS",
					...
					context:AddRegistered(
						"SAVE_SCRIPT",
						not presentClasses.isViableDecompileScript or not env.isdecompile or env.writefile == nil
					)
					context:AddRegistered(
						"SAVE_BYTECODE",
						...
						for _, v in next, selection.List do
							if v.Obj:IsA("LuaSourceContainer") and env.isViableDecompileScript(v.Obj) then
								...
								if not success or not source then
									source = ("-- DEX - %s failed to decompile %s"):format(
										env.executor,
										v.Obj.ClassName
									)
								end
					...
					if Settings.ScriptViewer.ShowMoreInfo then
						...
						if
							(
								...
								and (
									scr.RunContext == Enum.RunContext.Legacy
									or scr.RunContext == Enum.RunContext.Server
								)
							) or not scr:IsA("LocalScript")
						then
							source = source
						...
						elseif not env.isdecompile() then
							source = source
								.. "-- Reason: Your executor does not support decompiler. (missing 'decompile' function and 'getscriptbytecode' function as fallback)\n"
						else
	...
	Revival of the old dex's Save Instance
		...
		local function main()
			...
			SettingsWindow.Init = function()
				...
				local classIcon =
					AddDropdown("Class Icons", { "Old", "NewDark", "Vanilla3" }, Settings.ClassIcon, false, 100)
				...
				local copypathUseChildren =
					AddCheckbox("Use GetChildren to Copy Path", Settings.Explorer.CopyPathUseGetChildren)
				...
				local decompiler = AddDropdown(
					"Decompiler Fallback",
					decompilerOption,
					Settings.Decompiler.DecompilerFallback,
					false,
					125
				)
				...
				local ShinyPort =
					AddTextbox("Shiny Decompiler Port", tostring(Settings.Decompiler.ShinyDecompilerPort), 50)
				...
				local preferFallback =
					AddCheckbox("Prefer Fallback Decompiler", Settings.Decompiler.PreferDecompilerFallback)
...
local Main,
	...
	SaveInstance,
	ModelViewer,
	SettingsWindow, --[[, SecretServicePanel]]
	DefaultSettings,
	Notebook,
	Serializer,
	Lib
...
Main = (function()
	...
	Main.ModuleList =
		{ "Explorer", "Properties", "ScriptViewer", "Console", "SaveInstance", "ModelViewer", "SettingsWindow" }
	Main.Elevated = false
	...
	Main.InitEnv = function()
		...
		pcall(function()
			AdvancedDecompilerCache = loadstring(
				game:HttpGet(
					"https://raw.githubusercontent.com/"
						.. Main.GitName
						.. "/Advanced-Decompiler-V3/refs/heads/main/init.lua"
				)
			)()
		end)
		...
		if env.readfile and listfiles then
			if #listfiles("dex/plugins") > 0 then
				...
				for _, pluginDir in pairs(listfiles("dex/plugins")) do
					...
					local pluginFriendlyName = moduleData.PluginData.FriendlyName
						or moduleData.Window.GuiElems.Title.Text
						or "Unnamed Plugin"
					local pluginName = moduleData.PluginData.Name
						or moduleData.Window.GuiElems.Title.Text:gsub(" ", "")
						or "unnamedPlugin"
