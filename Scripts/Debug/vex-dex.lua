...
local function SerializeValue(Value)
	...
	elseif Type == "UDim2" then
		return `UDim2.new({FormatNumber(Value.X.Scale)}, {FormatNumber(Value.X.Offset)}, {FormatNumber(Value.Y.Scale)}, {FormatNumber(
			Value.Y.Offset
		)})`
	elseif Type == "Color3" then
		return `Color3.fromRGB({math.floor(Value.R * 255 + 0.5)}, {math.floor(Value.G * 255 + 0.5)}, {math.floor(
			Value.B * 255 + 0.5
		)})`
	...
	elseif Type == "Rect" then
		return `Rect.new({FormatNumber(Value.Min.X)}, {FormatNumber(Value.Min.Y)}, {FormatNumber(Value.Max.X)}, {FormatNumber(
			Value.Max.Y
		)})`
	...
	elseif Type == "NumberSequence" then
		...
		for _, K in Value.Keypoints do
			table.insert(
				Parts,
				`NumberSequenceKeypoint.new({FormatNumber(K.Time)}, {FormatNumber(K.Value)}, {FormatNumber(K.Envelope)})`
			)
		end
...
local function GetPropertyDiff(Object)
	local ClassGood, ClassName = pcall(function()
		return Object.ClassName
	end)
	if not ClassGood then
		return {}
	end
	...
	for _, PropName in Names do
		if SerializeSkip[PropName] then
			continue
		end
		if SerializeDeprecated[PropName] then
			continue
		end
		if ReadOnlyProperties and ReadOnlyProperties[PropName] then
			continue
		end
		...
		local ReadGood, Value = pcall(function()
		...
		local DefGood, Default = pcall(function()
			return Template[PropName]
		end)
		if not DefGood then
...
local function SerializeInstance(Object)
	...
	local AttrGood, Attrs = pcall(function()
...
local function InitialiseErrorLog()
	...
	if not makefolder or not isfolder or not isfile or not writefile then
		return
	end
...
local ClassToAsset = {
	...
	KeyframeSequence = "AnimationTrack",
	...
	RopeConstraint = "HingeConstraint",
	SpringConstraint = "HingeConstraint",
	RodConstraint = "HingeConstraint",
	PrismaticConstraint = "HingeConstraint",
	...
	CFrameValue = "NumberValue",
	Vector3Value = "NumberValue",
	Color3Value = "NumberValue",
	BrickColorValue = "NumberValue",
	RayValue = "NumberValue",
	ParticleEmitter = "ParticleEffect",
	Beam = "ParticleEffect",
	Trail = "ParticleEffect",
	Sparkles = "ParticleEffect",
	Fire = "Fire",
	Smoke = "Smoke",
	Explosion = "Explosion",
	PointLight = "Lighting",
	...
	Sky = "Sky",
	Clouds = "Sky",
	BloomEffect = "ColorCorrectionEffect",
	...
	BodyThrust = "BodyVelocity",
	AlignPosition = "BodyVelocity",
	AlignOrientation = "BodyVelocity",
	VectorForce = "BodyVelocity",
	Torque = "BodyVelocity",
	LineForce = "BodyVelocity",
	Dialog = "Chat",
	DialogChoice = "Chat",
...
local AssetCache = {}
local AssetsEnabled = (getcustomasset and getcustomasset or nil)
...
local function ResolveSelfForGlobal(Object, Method)
	if Method.SelfFrom == "Parent" then
		local Good, Parent = pcall(function()
...
local MethodGroups = {
	{
		...
		Methods = {
			...
			},
			{
				"replicatesignal",
				"void",
				{ { "signal", "RBXScriptSignal", "MouseButton1Click" }, { "args", "Variadic", "" } },
				"global",
				NoSelf = true,
			},
			{
...
local function RefreshThemePresetButton()
	if Explorer and Explorer.ThemePresetButton and Explorer.ThemePresetButton.Parent then
...
function VexUI:CreateWindow(Config)
	...
	Track(Services.UserInputService.InputEnded:Connect(function(Input)
		...
		if NewX ~= AbsX or NewY ~= AbsY then
			...
			VexUI:Tween(Window, {
				Position = UDim2.new(Cur.X.Scale, Cur.X.Offset + DeltaX, Cur.Y.Scale, Cur.Y.Offset + DeltaY),
			}, TweenSlide)
		end
	end))
	...
	Track(Services.UserInputService.InputChanged:Connect(function(Input)
		...
		Window.Position = UDim2.new(
			...
			ResizeStartUDim.X.Offset + OffsetXDelta,
			ResizeStartUDim.Y.Scale,
			ResizeStartUDim.Y.Offset + OffsetYDelta
		)
	end))
...
local function ResolveInstanceText(Text)
	...
	if Lowered == "" or Lowered == "character" or Lowered == "char" or Lowered == "hrp" or Lowered == "me" then
...
function Explorer:GetValueColor(Value)
	...
	if ValueType == "Color3" or ValueType == "BrickColor" or ValueType == "ColorSequence" then
		return Theme.PropString
	end
...
function Explorer:BeginRename(Node)
	if not Node or not Node.Label or not Node.Label.Parent then
		return
	end
...
function Explorer:_FlatRefreshVisibleSelection()
	if not self._FlatResultsPool then
		return
	end
...
function Explorer:_VTreeRefreshRowMeta(RowData)
	if RowData.IsNilContainer then
		local Count = 0
		local Good, List = pcall(function()
			return self:_CollectNilInstances()
		end)
...
function Explorer:_VTreeBeginRename(PoolEntry, RowData)
	if not PoolEntry or not RowData or not RowData.Instance then
		return
	end
	if RowData.IsNilContainer then
		return
	...
	local GoodOld, OldText = pcall(function()
		return Inst.Name
	end)
	if not GoodOld or type(OldText) ~= "string" then
		OldText = RowData.RawName or ""
	end
...
function Explorer:_VTreeAllocateRow(Container)
	...
	ArrowHit.MouseButton1Click:Connect(function()
		local RowData = PoolEntry.Bound
		if not RowData or not RowData.HasChildren then
			return
		end
	...
	Row.MouseButton1Click:Connect(function()
		...
		if self._JustDragged or self.JustDragged then
			return
		end
		...
		if Now - LastClickAt < 0.35 then
			...
			elseif Cls == "Script" then
				local Good, RC = pcall(function()
					return Inst.RunContext
				end)
		...
		if self.ShiftHeld and self.SelectionAnchor then
			...
			for I = 1, #Rows do
				...
				if RI == self.SelectionAnchor then
					AnchorIdx = I
				end
				if RI == Inst then
					TargetIdx = I
			...
			if AnchorIdx and TargetIdx then
				if AnchorIdx > TargetIdx then
					AnchorIdx, TargetIdx = TargetIdx, AnchorIdx
				end
				local NewSel = {}
				for I = AnchorIdx, TargetIdx do
					local RI = Rows[I].Instance
					if RI then
						table.insert(NewSel, RI)
					end
				end
	...
	Row.InputBegan:Connect(function(Input)
		...
		then
			local RowData = PoolEntry.Bound
			if not RowData or not RowData.Instance or RowData.IsNilContainer then
				return
			end
			self.DragOperation = {
	...
	Row.MouseButton2Click:Connect(function()
		local RowData = PoolEntry.Bound
		if not RowData or not RowData.Instance then
			return
		end
...
function Explorer:_VTreeBindRow(PoolEntry, RowData, YPos)
	...
	if RowData.Instance and not RowData.IsNilContainer then
		local GoodN, FreshName = pcall(function()
			return RowData.Instance.Name
		end)
...
function Explorer:_VTreeScheduleRebuild()
	if self._VTreeRebuildScheduled then
		return
	end
...
function Explorer:_VTreeHookScroll()
	...
	self._VTreeScrollConn = Scroll:GetPropertyChangedSignal("CanvasPosition"):Connect(function()
		...
		if self._VTreeFilterActive and self._VTreeFilteredResume and not self._VTreeFilteredExhausted then
...
function Explorer:_VTreeExpand(RowData)
	if not RowData or RowData.Expanded then
		return
	end
	if not RowData.HasChildren then
		return
	end
...
function Explorer:_VTreeCollapse(RowData)
	if not RowData or not RowData.Expanded then
		return
	end
...
function Explorer:_VTreeRevealInstance(Inst, Options)
	...
	local GoodP, Cur = pcall(function()
		return Inst.Parent
	end)
	if GoodP then
		Cur = Cur
	...
	while Cur and Cur ~= game do
		...
		local GoodCur, P = pcall(function()
			return Cur.Parent
		end)
		if not GoodCur then
			break
	...
	for _, Ancestor in Chain do
		local Idx = self._VTreeRowsByInstance[Ancestor] or self:_VTreeResolveRowIndex(Ancestor)
		if Idx then
	...
	local TargetIdx = self._VTreeRowsByInstance[Inst] or self:_VTreeResolveRowIndex(Inst)
...
function Explorer:_VTreeInsertChildRow(ParentInst, NewChild)
	if not ParentInst or not NewChild then
		return
	end
	if self._VTreeRowsByInstance[NewChild] then
		return
	end
...
function Explorer:_VTreeRemoveInstanceRow(Inst)
	...
	if not Index then
		...
		if not GoodParent or not ParentInst then
			return
		end

		local ParentIndex = self._VTreeRowsByInstance[ParentInst] or self:_VTreeResolveRowIndex(ParentInst)
		if not ParentIndex then
			return
		end
		...
		for I = ParentIndex + 1, #Rows do
			local R = Rows[I]
			if R.Depth <= PDepth then
				break
			end
			if R.Depth == PDepth + 1 then
				StillHasKids = true
				break
			end
		end
		...
		if not StillHasKids then
			local GoodKids, Kids = pcall(function()
				return WeakGetChildren(ParentInst)
			end)
			local HasKids = false
	...
	local ParentInst
	local GoodParent, P = pcall(function()
		return Inst.Parent
	end)
	if GoodParent then
		ParentInst = P
...
function Explorer:_VTreeHookAncestryFor(Inst)
	...
	if self._VTreeAncestryConns[Inst] then
		return
	end

	local GoodSig, Sig = pcall(function()
		return Inst.AncestryChanged
	end)
	if not GoodSig or not Sig then
		return
	end
	...
	self._VTreeAncestryConns[Inst] = Track(Sig:Connect(function(_, NewParent)
		task.defer(function()
			...
			local GoodP, RealParent = pcall(function()
				return Inst.Parent
			end)
			if not GoodP then
				return
			...
			local ParentIdx = self._VTreeRowsByInstance[RealParent] or self:_VTreeResolveRowIndex(RealParent)
			if ParentIdx then
...
function Explorer:_VTreeHookGameSignals()
	if self._VTreeGameSignalsHooked then
		return
	end
	...
	self._VTreeAddedConn = Track(game.DescendantAdded:Connect(function(Inst)
		task.defer(function()
			...
			if not GoodParent or not Parent then
				return
			end

			if Parent == game then
				if self._VTreeRowsByInstance[Inst] then
					return
				end
			...
			local ParentIdx = self._VTreeRowsByInstance[Parent] or self:_VTreeResolveRowIndex(Parent)
			if ParentIdx then
				local CanonicalParent = self._VTreeRows[ParentIdx].Instance
	...
	self._VTreeRemovingConn = Track(game.DescendantRemoving:Connect(function(Inst)
		...
		if self._VTreeNameConns and self._VTreeNameConns[Inst] then
			pcall(function()
				self._VTreeNameConns[Inst]:Disconnect()
			end)
		...
		if self._VTreeAncestryConns and self._VTreeAncestryConns[Inst] then
			pcall(function()
				self._VTreeAncestryConns[Inst]:Disconnect()
			end)
	...
	for I = #Chain, 1, -1 do
		...
		if not CurrentIdx then
			for J = 1, #self._VTreeRows do
				...
				if Row.Depth == 0 and not Row.IsNilContainer and Row.RawName == SegName then
					CurrentIdx = J
					CurrentDepth = 0
					break
				end
...
function Explorer:_VTreeBuildFiltered()
	...
	for Inst in MatchSet do
		if Inst then
			...
			while Cursor and Cursor ~= game and Safety < 64 do
				...
				Include[Cursor] = true
				if Prev then
					AddChild(Cursor, Prev)
				end
				if Already then
					break
				end
				Prev = Cursor
				local GoodP, P = pcall(function()
					return Cursor.Parent
				end)
	...
	local RootList = {}
	for Inst in Roots do
		RootList[#RootList + 1] = Inst
	end
	...
	local function MakeRow(Inst, Depth, HasKids)
		local GoodN, N = pcall(function()
			return Inst.Name
		end)
		local GoodC, C = pcall(function()
			return Inst.ClassName
		end)
		return {
			Instance = Inst,
	...
	local function Resume()
		if Token ~= Self._VTreeFilterBuildToken then
			return
		end
...
function Explorer:_VTreeRestoreScroll()
	...
	if not self._VTreeSavedScrollY then
		return
	end
...
function Explorer:_CollectNilInstances()
	...
	for _, Inst in List do
		...
		local GoodP, Parent = pcall(function()
			return Inst.Parent
		end)
		...
		local GoodAnc, IsDesc = pcall(function()
		...
		if ClassFilter then
			local GoodC, ClassName = pcall(function()
				return Inst.ClassName
			end)
			if not GoodC or not ClassName then
				continue
			end
...
function Explorer:_VTreeStartNilStream(ContainerRow)
	...
	local function EmitPage()
		if Token ~= Self._VTreeNilStreamToken then
			return
		end
		if not ContainerRow.Expanded then
			return
		end
	...
	for _, Op in _ComparisonOperators do
		...
		if SplitPos then
			...
			return {
				...
				BooleanValue = (RawValue:lower() == "true") and true or (RawValue:lower() == "false") and false or nil,
			}
		end
	end
...
function Explorer:_CompareValues(Actual, Parsed)
	...
	if Op == ">" then
		return Numeric > Parsed.NumericValue
	end
	if Op == "<" then
		return Numeric < Parsed.NumericValue
	end
...
function Explorer:ClearSearchAndJumpTo()
	...
	task.defer(function()
		...
		local Idx = self._VTreeRowsByInstance[Target] or self:_VTreeResolveRowIndex(Target)
		if not Idx then
			return
		end
...
function Explorer:RangeSelect(AnchorInstance, EndInstance)
	...
	for I, Row in Rows do
		if Row.Instance == AnchorInstance then
			AnchorIdx = I
		end
		if Row.Instance == EndInstance then
			EndIdx = I
		end
		if AnchorIdx and EndIdx then
			break
		end
	end
...
function Explorer:OnSelectionChanged()
	...
	if self.SelectedOrder then
		for _ in self.SelectedOrder do
			Count += 1
		end
	end
...
function Explorer:CreateEnumRow(Object, PropertyName, Value, Parent)
	...
	Button.MouseButton1Click:Connect(function()
		...
		self:OpenListModal(EnumName:upper(), Items, function(Item)
			return Item.Name
	...
	Handle(function()
		...
		if self.PropertiesTitleLabel then
			if #self.SelectedOrder > 1 then
				self.PropertiesTitleLabel.Text =
					`{Object.ClassName}  -  {Object.Name}   ({#self.SelectedOrder} selected)`
			else
		...
		if #Attributes > 0 or Filter == "" then
			...
			local AttributeButtonLayout = VexUI:AddListLayout(AttributeButtonRow, 4, Enum.FillDirection.Horizontal)
			...
			CreateAttributeButton("Copy", function()
				local AttrGood, AttrMap = pcall(function()
					return Object:GetAttributes()
				end)
		...
		VexUI:AddListLayout(TagButtonRow, 6, Enum.FillDirection.Horizontal)
...
function Explorer:CreateModalWindow(Title, Width, Height, Options)
	...
	if Options.Resizable then
		...
		MakeEdge(
			UDim2.new(1, -EdgeThickness, 0, EdgeThickness),
			UDim2.new(0, EdgeThickness, 1, -EdgeThickness * 2),
			1,
			0
		)
		MakeEdge(UDim2.new(0, 0, 0, EdgeThickness), UDim2.new(0, EdgeThickness, 1, -EdgeThickness * 2), -1, 0)
		MakeEdge(
			UDim2.new(0, EdgeThickness, 1, -EdgeThickness),
			UDim2.new(1, -EdgeThickness * 2, 0, EdgeThickness),
			0,
			1
		)
...
function Explorer:OpenColorPicker(CurrentColor, Callback, Options)
	...
	CreateChannelSlider("R", Theme.Red, function()
		return R
	end, function(v)
		R = v
	end, 4)

	CreateChannelSlider("G", Theme.Green, function()
		return G
	end, function(v)
		G = v
	end, 5)

	CreateChannelSlider("B", Theme.Blue, function()
		return B
	end, function(v)
		B = v
	end, 6)
...
function Explorer:OpenListModal(
	Title,
	Items,
	ItemTextFunction,
	OnPick,
...
function Explorer:SelectChildrenOfSelection()
	...
	for _, Parent in Sources do
		local Idx = self._VTreeRowsByInstance[Parent] or self:_VTreeResolveRowIndex(Parent)
		if Idx then
			local Row = self._VTreeRows[Idx]
			if Row and Row.HasChildren and not Row.Expanded then
				self:_VTreeExpand(Row)
...
function Explorer:ParseAttributeValue(AttributeType, InputText)
	...
	if AttributeType == "Vector3" then
		local XValue, YValue, ZValue = Text:match("([%-%d%.]+)[,%s]+([%-%d%.]+)[,%s]+([%-%d%.]+)")

		if XValue then
			return Vector3.new(tonumber(XValue), tonumber(YValue), tonumber(ZValue))
		end
	elseif AttributeType == "Vector2" then
		local XValue, YValue = Text:match("([%-%d%.]+)[,%s]+([%-%d%.]+)")

		if XValue then
			return Vector2.new(tonumber(XValue), tonumber(YValue))
		end
	elseif AttributeType == "Color3" then
		local RedValue, GreenValue, BlueValue = Text:match("([%-%d%.]+)[,%s]+([%-%d%.]+)[,%s]+([%-%d%.]+)")
	...
	elseif AttributeType == "UDim" then
		local ScaleValue, OffsetValue = Text:match("([%-%d%.]+)[,%s]+([%-%d]+)")

		if ScaleValue then
			return UDim.new(tonumber(ScaleValue), tonumber(OffsetValue))
		end
	...
	elseif AttributeType == "CFrame" then
		...
		if #NumberValues == 3 then
			return CFrame.new(NumberValues[1], NumberValues[2], NumberValues[3])
		end
	elseif AttributeType == "NumberRange" then
		local MinimumValue, MaximumValue = Text:match("([%-%d%.]+)[,%s]+([%-%d%.]+)")

		if MinimumValue then
			return NumberRange.new(tonumber(MinimumValue), tonumber(MaximumValue))
		end
	end
...
function Explorer:OpenAttributeModal(Title, ExistingName, ExistingValue)
	...
	PositionTypeDropdown = function()
		...
		TypeDropdown.Position =
			UDim2.fromOffset(ButtonPosition.X - LayerPosition.X, ButtonPosition.Y - LayerPosition.Y + ButtonSize.Y + 2)

		TypeDropdown.Size = UDim2.fromOffset(ButtonSize.X, #AttributeTypes * 22)
	end
...
function Explorer:OpenRemoveAttributeModal()
	...
	self:OpenListModal("Remove Attribute", Items, function(e)
...
function Explorer:OpenEditAttributeModal()
	...
	self:OpenListModal("Edit Attribute", Items, function(e)
		return `{e.Name}  ({typeof(e.Value)})`
	end, function(e)
...
function Explorer:OpenRemoveTagModal()
	...
	self:OpenListModal("Remove Tag", Tags, function(Tag)
		return tostring(Tag)
	end, function(Tag)
		for _, Obj in self:GetSelectionList() do
			pcall(function()
				Services.CollectionService:RemoveTag(Obj, Tag)
			end)
		end
...
function Explorer:Open3DPreview(Instance)
	...
	local function UpdateCamera()
		...
		Camera.CFrame = CFrame.lookAt(CenterPosition + Vector3.new(OffsetX, OffsetY, OffsetZ), CenterPosition)
	end
...
function Explorer:OpenCallFunction()
	...
	self:OpenListModal("Call Function", Methods, function(Method)
		local Params = {}
...
function Explorer:OpenMethodCaller(Method)
	...
	local function ParseArg(ArgType, Text)
		...
		if ArgType == "RBXScriptSignal" then
			local Good, Signal = pcall(function()
				return Object[Text]
			end)
			if not Good then
	...
	local function SerializeOutput(Value)
		...
		if ValueType == "Instance" then
			local Good, Path = pcall(function()
				return BuildLuaPath(Value)
			end)
		...
		local Good, Formatted = pcall(function()
...
function Explorer:ShowConnectionsResult(Connections)
	...
	local Window, Body =
		self:CreateModalWindow(`Connections ({#Connections})`, 540, math.min(560, 80 + #Connections * 68))
	...
	for Index, Conn in Connections do
		...
		local Header = VexUI:CreateInstance("TextLabel", {
			...
			BackgroundTransparency = 1,
			Font = Fonts.Mono,
			Text = `[{Index}]  Enabled={tostring(Enabled)}   Foreign={tostring(ForeignState)}   Lua={tostring(
				LuaConnection
			)}`,
			TextColor3 = Theme.Text,
			TextSize = 11,
		...
		local function RefreshEnabled()
			...
			Header.Text = `[{Index}]  Enabled={tostring(NewEnabled)}   Foreign={tostring(ForeignState)}   Lua={tostring(
				LuaConnection
			)}`
		end
		...
		MakeBtn("Disable", 60, function()
			...
			if Good then
				...
				self:Notify(`Disable failed: {Err}`)
			end
		end)
...
function Explorer:DumpScriptFunctions(ScriptObject)
	...
	local function FormatValue(Value)
		...
		if ValueType == "function" then
			...
			return string.format("<function %s @ %s:%s>", Name ~= "" and Name or "?", Source, Line)
		end
	...
	Push(string.format("-- Function dump for %s", ScriptObject:GetFullName()))

	Push(string.format("-- Functions found: %d", #Functions))
	...
	for Index, Fn in Functions do
		...
		Push("")
		Push(string.format("[%d] %s @ %s:%s", Index, Name ~= "" and Name or "(anonymous)", Source, Line))
			...
			if Success and type(Constants) == "table" then
				if next(Constants) == nil then
					...
					for ConstantIndex, ConstantValue in Constants do
						Push(string.format("    [%s] = %s", tostring(ConstantIndex), FormatValue(ConstantValue)))
					end
				end
			else
				Push(string.format("  Constants: <unavailable> (%s)", tostring(Constants)))
			end
		end
			...
			if Success and type(Upvalues) == "table" then
				if next(Upvalues) == nil then
					...
					for UpvalueIndex, UpvalueValue in Upvalues do
						Push(string.format("    [%s] = %s", tostring(UpvalueIndex), FormatValue(UpvalueValue)))
					end
				end
			else
				Push(string.format("  Upvalues: <unavailable> (%s)", tostring(Upvalues)))
			end
		end
	end
...
function Explorer:OpenConstantUpvalueSearch(ScriptObject)
	...
	ConstsBtn.MouseButton1Click:Connect(function()
		SetConstsOn(not GetConstsOn())
	end)
	UpsBtn.MouseButton1Click:Connect(function()
		SetUpsOn(not GetUpsOn())
	end)
...
function Explorer:OpenScriptViewer(ScriptObject, UseDefault)
	...
	if not Source then
		Source =
			`-- Failed to decompile {ScriptObject.ClassName} "{ScriptObject.Name}"\n-- Your executor probably does not support decompile.`
	end
	...
	DumpScriptButton = MakeStripButton("Dump Script", 86, function()
		...
		if writefile and isfolder and makefolder then
			FileOK = pcall(function()
				if not isfolder("vex_dumps") then
					makefolder("vex_dumps")
				end
	...
	DumpBytecodeButton = MakeStripButton("Dump Bytecode", 102, function()
		...
		self:Notify(FileOK and `Dumped bytecode ({#Bytecode} bytes) -> {Path}` or "Bytecode dump failed (file)")
	end)
	...
	local function HighlightSafe(Text)
		...
		if not Good or type(Result) ~= "string" then
...
function Explorer:OpenConsole()
	...
	local Filters = self._ConsoleFilters
		or {
	...
	local function MakeFilterButton(LabelText, MessageType)
		local Btn = CreateTopBarButton(LabelText, function()
			...
			FilterButtons[MessageType].TextColor3 = Filters[MessageType] and ColorForType(MessageType)
				or Theme.TextFaded
			RefreshVisibility()
		end)
	...
	local function FormatArguments(...)
		...
		local Parts = {}
		for I = 1, N do
			Parts[I] = FormatArgument(select(I, ...))
		end
	...
	local function ExecuteCommand()
		...
		local function CapturedPrint(...)
			PushSynthetic(FormatArguments(...), Enum.MessageType.MessageOutput)
		end
		local function CapturedWarn(...)
			PushSynthetic(FormatArguments(...), Enum.MessageType.MessageWarning)
		end
...
function Explorer:OpenSettings()
	...
	local function CreateSlider(
		...
		local function Update(InputX)
			local NewFraction = math.clamp((InputX - TrackFrame.AbsolutePosition.X) / TrackFrame.AbsoluteSize.X, 0, 1)
	...
	table.sort(AllFonts, function(A, B)
...
function Explorer:OpenFiltersDropdown(AnchorButton)
	...
	FiltersClickConn = Services.UserInputService.InputBegan:Connect(function(Input)
		...
		if Pos.X >= AbsP.X and Pos.X <= AbsP.X + AbsS.X and Pos.Y >= AbsP.Y and Pos.Y <= AbsP.Y + AbsS.Y then
			return
		end
	...
	CreateSection("Nil Instances", 1, function(Container)
		...
		CreateToggleRow(Inner, "Show Nil Instances Folder", true, 1, nil, function()
			return not self.HideNilContainer
		end, function()
			self:ToggleNilContainerFilter()
		end)

		CreateToggleRow(Inner, "Search Nil Instances", true, 2, nil, function()
	...
	CreateSection("Freeze Search", 2, function(Container)
		...
		local Box = VexUI:CreateInstance("TextBox", {
			...
			BorderSizePixel = 0,
			Font = Fonts.Mono,
			PlaceholderText = self.FreezeSearchMatchClass and "e.g. MeshPart" or "e.g. SkibidiPartMesh",
			PlaceholderColor3 = Theme.TextFaded,
		...
		BindTheme("Field", function(C)
			if Box.Parent then
				Box.BackgroundColor3 = C
			end
		end)
		BindTheme("Text", function(C)
			if Box.Parent then
				Box.TextColor3 = C
			end
		end)
		BindTheme("TextFaded", function(C)
			if Box.Parent then
				Box.PlaceholderColor3 = C
			end
		end)
		...
		CreateToggleRow(Inner, "Match by ClassName (instead of Name)", true, 3, nil, function()
			...
			if self.FreezeSearchBox and self.FreezeSearchBox.Parent then
				self.FreezeSearchBox.PlaceholderText = self.FreezeSearchMatchClass and "e.g. MeshPart"
	...
	CreateSection("Search Behaviour", 3, function(Container)
		...
		CreateToggleRow(Inner, "Match by ClassName", true, 1, nil, function()
			return self.MatchByClassName == true
		end, function()
			self:ToggleMatchByClassName()
		end)

		CreateToggleRow(Inner, "Match by Property (use Prop=Value syntax)", true, 2, nil, function()
			return self.MatchByProperty == true
		end, function()
			self:ToggleMatchByProperty()
		end)

		CreateToggleRow(Inner, "Show only matches (flat list)", true, 3, nil, function()
	...
	CreateSection("Search - ClassName Filter", 4, function(Container)
		...
		for Index, ClassName in self.FilterClassOptions do
			CreateToggleRow(Grid, ClassName, false, Index, ClassName, function()
	...
	CreateSection("Hidden Services", 5, function(Container)
		...
		CreateToggleRow(Inner, "Hide All Services", true, 1, nil, function()
		...
		for Index, Service in ServiceList do
			...
			CreateToggleRow(Grid, ServiceName, false, Index, Service.ClassName, function()
...
function Explorer:InitFreezeState()
	if self._FreezeInitialized then
		return
	end
...
function Explorer:_StartFreezeSearchWatcher()
	...
	local Good, Conn = pcall(function()
		return game.DescendantAdded:Connect(function(RawDescendant)
			if typeof(RawDescendant) ~= "Instance" then
				return
			end
			if self.ExplorerFrozen then
				return
			...
			task.defer(function()
				...
				local Reason = MatchByClass and `matched class "{TargetField}"` or `matched "{TargetField}"`
...
function Explorer:SetFreezeSearchTerm(Term)
	...
	if self.FreezeSearchBox and self.FreezeSearchBox.Parent and self.FreezeSearchBox.Text ~= Term then
...
function Explorer:_StartFreezeWatcher()
	...
	self._FreezeWatcherConnection = Track(game.DescendantAdded:Connect(function(Descendant)
		...
		if not Good or type(Name) ~= "string" then
			return
		end
...
function Explorer:BuildQuickAccessBar()
	...
	local function ApplyTileVisual(Tile, IsActive, Hovering)
		...
		local TargetTransparency = Hovering and math.clamp(BaseTransparency - 0.1, 0, 1) or BaseTransparency
	...
	local function OpenDropdown()
		...
		MakeTile(
			...
			"Explorer",
			function()
				return self.ExplorerWindow and self.ExplorerWindow.Frame and self.ExplorerWindow.Frame.Visible ~= false
			end,
			1,
			function()
...
function Explorer:_ResolvePinPath(Path)
	...
	for I, Seg in Segments do
		...
		local Good, Next = pcall(function()
			return Cur:FindFirstChild(Seg)
		end)
		if not Good or not Next then
			return nil
		end
		Cur = Next
	end
...
function Explorer:_RebuildPinBar()
	...
	for I, Path in self.PinnedPaths do
		...
		local ClassName
		if Inst then
			local GN, N = pcall(function()
				return Inst.Name
			end)
			DisplayName = GN and N or "?"
			local GC, C = pcall(function()
				return Inst.ClassName
			end)
...
function Explorer:BuildExplorerWindow()
	...
	local FreezeButton = self.ExplorerWindow:AddTitleButton("", 24, false, function()
		self:ToggleExplorerFreeze()
	end, "FreezeIcon", 10, 18)
...
function Explorer:SetupDragHandlers()
	...
	Track(Services.UserInputService.InputChanged:Connect(function(Input)
		...
		if DragOperation.HasStarted and DragOperation.GhostLabel then
			DragOperation.GhostLabel.Position = UDim2.fromOffset(Input.Position.X + 12, Input.Position.Y + 8)
		end
	end))
	...
	Track(Services.UserInputService.InputEnded:Connect(function(Input)
			...
			for _, PoolEntry in self._VTreePool do
				...
				if Row and Row.Visible and RowData and RowData.Instance and not RowData.IsNilContainer then
					...
					if
						AS.Y > 0
						and MouseX >= AP.X
						and MouseX <= AP.X + AS.X
						and MouseY >= AP.Y
						and MouseY <= AP.Y + AS.Y
					then
		...
		for _, SourceInstance in SourceInstances do
			local Success = pcall(function()
				if SourceInstance ~= TargetInstance and not TargetInstance:IsDescendantOf(SourceInstance) then
...
function Explorer:_ValuesEqual(A, B)
	...
	local T = typeof(A)
	if T == "Color3" or T == "Vector3" or T == "Vector2" or T == "UDim2" or T == "UDim" or T == "CFrame" then
		return tostring(A) == tostring(B)
	end
	return false
end
...
function Explorer:OpenContextMenu(AnchorX, AnchorY)
	...
	local HasSearch = (self._LastAppliedSearchQuery or "") ~= "" or (self.SearchBox and self.SearchBox.Text ~= "")
	...
	local CanTeleport = self.SelectedInstance
		and (
			self.SelectedInstance:IsA("BasePart")
			or self.SelectedInstance:IsA("Model")
			or self.SelectedInstance:IsA("Tool")
			or self.SelectedInstance:IsA("Attachment")
		)
	...
	if not IsScript and PrimaryClass == "Script" and self.SelectedInstance then
		local Success, RunContext = pcall(function()
	...
	if typeof(Data.Fonts) == "table" then
		for Key, Name in Data.Fonts do
			if Fonts[Key] and typeof(Name) == "string" then
				local Good, FontEnum = pcall(function()
...
function Explorer:FetchVersion()
	local Good, Result = pcall(function()
		return loadstring(
			game:HttpGet("https://raw.githubusercontent.com/Vezise/2026/main/Vez/VexExplorer/VexVersion.lua")
		)()
	end)
