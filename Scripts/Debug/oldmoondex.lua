			...
			for i, v in pairs(newMt.Markers) do
				if i < newMt.TotalSpace then
					CreateInstance("Frame", {
						...
						BackgroundColor3 = v,
						BorderSizePixel = 0,
						Position = hor and UDim2.new(i / newMt.TotalSpace, 0, 1, -6)
							or UDim2.new(1, -6, i / newMt.TotalSpace, 0),
			...
			for i = 1, entrySpace do
				...
				if node then
					...
					if not entry then
						...
						entry.Position =
							UDim2.new(0, self.OffX, 0, self.OffY + (self.Height + 1) * #displayFrame:GetChildren())
...
function f.hookWindowListener(window)
	...
	window.TopBar.InputBegan:connect(function(input)
		if input.UserInputType == Enum.UserInputType.MouseButton1 then
			...
			local dragOffX, dragOffY =
				mouse.X - window.TopBar.AbsolutePosition.X, mouse.Y - window.TopBar.AbsolutePosition.Y
			local inPane = false
			local releaseEvent
			local mouseEvent
			...
			mouseEvent = user.InputChanged:Connect(function(input)
				if
					input.UserInputType == Enum.UserInputType.MouseMovement
					and (selectedInit or f.distance(initPos[1], initPos[2], mouse.X, mouse.Y) >= (inPane and 20 or 5))
				then
...
local function welcomePlayer()
	...
	welcomeChangelog:TweenPosition(
		UDim2.new(0.6, 5, 0, 20),
		Enum.EasingDirection.Out,
		Enum.EasingStyle.Quart,
		0.5,
		true
	...
	contentR:TweenPosition(
		UDim2.new(1, -explorerSettings.RPaneWidth, 0, 0),
		Enum.EasingDirection.Out,
		Enum.EasingStyle.Quart,
		0.5,
		true
