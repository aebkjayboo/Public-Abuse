...
local function getTorso(char)
	return char and (char:FindFirstChild("UpperTorso") or char:FindFirstChild("Torso"))
end
...
function flingManager.SetFlingVelocity(part)
	...
	pcall(function()
		part.AssemblyLinearVelocity = flingManager.FlingVelocity
	end)
	pcall(function()
		part.AssemblyAngularVelocity = flingManager.FlingAngularVelocity
	end)
	pcall(function()
		part.Velocity = flingManager.FlingVelocity
	end)
	pcall(function()
		part.RotVelocity = flingManager.FlingAngularVelocity
	end)
end
...
local function fling(targetPlayer)
		...
		repeat
			if root and tHum then
				...
				if baseSpeed < 50 then
					...
					FPos(
						basePart,
						CFrame.new(0, 1.5, 0) + tHum.MoveDirection * baseSpeed / 1.25,
					...
					FPos(
						basePart,
						CFrame.new(2.25, 1.5, -2.25) + tHum.MoveDirection * baseSpeed / 1.25,
						CFrame.Angles(math.rad(angle), 0, 0)
					)
					task.wait()
					FPos(
					...
					FPos(basePart, CFrame.new(0, 1.5, tHum.WalkSpeed), CFrame.Angles(math.rad(90), 0, 0))
					task.wait()
					FPos(basePart, CFrame.new(0, 1.5, tRootSpeed / 1.25), CFrame.Angles(math.rad(90), 0, 0))
					task.wait()
					FPos(basePart, CFrame.new(0, -1.5, -tRootSpeed / 1.25), CFrame.Angles(0, 0, 0))
					task.wait()
					FPos(basePart, CFrame.new(0, 1.5, tRootSpeed / 1.25), CFrame.Angles(math.rad(90), 0, 0))
					task.wait()
	...
	repeat
		...
		pcall(function()
			char:PivotTo(returnCF)
		end)
		hum:ChangeState(Enum.HumanoidStateType.GettingUp)
		for _, p in char:GetChildren() do
			if p:IsA("BasePart") then
				flingManager.ClearPartVelocity(p)
			end
		end
		task.wait()
...
local function parseFlingSpeed(value)
	...
	if not speed or speed ~= speed or speed <= 0 then
		speed = 10000
	end
...
local function startWalkFling(speed)
	...
	walkflingConn = RunService.Heartbeat:Connect(function()
		...
		if r then
			walkFlingBurst(r, walkflingSpeed)
		end
	end)
end
...
local function startClickFling()
	...
	if clickflingConn then
		clickflingConn:Disconnect()
	end
	clickflingConn = mouse.Button1Down:Connect(function()
		if not clickflingActive then
			return
		end
