...
local _utf8char = utf8 and utf8.char
	or function(cp)
		if cp < 128 then
...
local function validate_utf8(input, toml_sub)
	...
	while i <= len do
			...
			if toml_sub then
				if byte < 9 then
					return false,
						line_number,
						line_number_start,
						"TOML only allows some control characters, but they must be escaped in double quoted strings"
				elseif byte == chars.CR and sbyte(input, i + 1) ~= chars.LF then
					return false, line_number, line_number_start, "TOML requires all '\\r' be followed by '\\n'"
		...
		elseif byte == 224 then
			second = sbyte(input, i + 1)
			third = sbyte(input, i + 2)

			if second ~= nil and second >= 128 and second <= 159 then
		...
		elseif byte == 237 then
			...
			if second ~= nil and second >= 160 and second <= 191 then
				return false, line_number, line_number_start, "Invalid UTF-8 Sequence"
			end
			i = i + 3
		...
		elseif byte == 240 then
			second = sbyte(input, i + 1)
			third = sbyte(input, i + 2)
			fourth = sbyte(input, i + 3)

			if second ~= nil and second >= 128 and second <= 143 then
				return false, line_number, line_number_start, "Invalid UTF-8 Sequence"
			end
			i = i + 4
		...
		if third == nil or third < 128 or third > 191 then
			return false, line_number, line_number_start, "Invalid UTF-8 Sequence"
		end
		if fourth == nil or fourth < 128 or fourth > 191 then
			return false, line_number, line_number_start, "Invalid UTF-8 Sequence"
		end
	end
	return true
...
local function handle_backslash_escape(sm)
	...
	if not sm.match then
		sm._, sm.end_seq, sm.match, sm.ext = sm.input:find(
...
local function close_string(sm)
	...
	if second == chars.DOUBLE_QUOTE and third == chars.DOUBLE_QUOTE then
		if sm.mode == "table" then
			_error(sm, "Cannot have multiline strings as table keys", "table")
		end
	...
	while found_closing_quote == false and sm.i <= sm.input_length do
		...
		if sm.byte == chars.BACKSLASH then
			...
			if escape ~= nil then
				...
				_error(
					sm,
					"TOML only allows specific escape sequences. Invalid escape sequence found: '\\" .. sm.match .. "'",
					"string"
				)
			end
		...
		elseif sm.multiline_string then
			...
			elseif sm.byte == chars.CR or sm.byte == chars.LF then
				_error(
					sm,
					"String does not appear to be closed. Use multi-line (triple quoted) strings if non-escaped newlines are desired.",
					"string"
				)
			end
		end
...
local function close_bare_string(sm)
	...
	if sm.match then
		...
		_error(
			sm,
			"Bare keys can only contain 'a-zA-Z0-9-_'. Invalid bare key found: '"
				.. sm.input:sub(sm.input:find("[^ #\r\n,]+", sm.i))
				.. "'",
			"keys"
		)
	end
...
local function remove_underscores_number(sm, number, anchor)
	if number:find("_") then
		if number:find("__") then
			_error(
				sm,
				"Numbers cannot have consecutive underscores. Found " .. anchor .. ": '" .. number .. "'",
				anchor
			)
		end
		if number:find("^_") or number:find("_$") then
			_error(
				sm,
				"Underscores are not allowed at beginning or end of a number. Found "
					.. anchor
					.. ": '"
					.. number
					.. "'",
		...
		if number:find("%D_%d") or number:find("%d_%D") then
			_error(
				sm,
				"Underscores must have digits on either side. Found " .. anchor .. ": '" .. number .. "'",
				anchor
			)
		end
...
local function validate_integer(sm, value)
	...
	if sm.match then
		if sm.match:find("^[-+]?0[%d_]") then
			_error(sm, "Integers can't start with a leading 0. Found integer: '" .. sm.match .. "'", "integer")
		end
...
local function validate_float(sm, value)
	...
	if sm.match then
		if sm.match:find("%._") or sm.match:find("_%.") then
			_error(
				sm,
				"Underscores in floats must have a number on either side. Found float: '" .. sm.match .. sm.ext .. "'",
				"float"
			)
		end
		if sm.match:find("^[-+]?0[%d_]") then
			_error(sm, "Floats can't start with a leading 0. Found float: '" .. sm.match .. sm.ext .. "'", "float")
		end
	...
	if sm.match then
		if sm.match:find("_[eE]") or sm.match:find("[eE]_") then
			_error(
				sm,
				"Underscores in floats cannot be before or after the e. Found float: '" .. sm.match .. sm.ext .. "'",
				"float"
			)
		end
		sm.match = remove_underscores_number(sm, sm.match, "float")
...
local function validate_seconds(sm, sec, anchor)
	if sec > 60 then
		_error(sm, "Seconds must be less than 61. Found second: " .. sec .. " in: '" .. sm.match .. "'", anchor)
	end
end

local function validate_hours_minutes(sm, hour, min, anchor)
	if hour > 23 then
		_error(sm, "Hours must be less than 24. Found hour: " .. hour .. " in: '" .. sm.match .. "'", anchor)
	end
	if min > 59 then
		_error(sm, "Minutes must be less than 60. Found minute: " .. min .. " in: '" .. sm.match .. "'", anchor)
	end
end

local function validate_month_date(sm, year, month, day, anchor)
	if month == 0 or month > 12 then
		_error(sm, "Month must be between 01-12. Found month: " .. month .. " in: '" .. sm.match .. "'", anchor)
	end
			...
			if day > 28 then
				_error(
					...
					"Too many days in month. Found "
						.. day
						.. " days in February, which only has 28 days if it's not a leap year in: '"
						.. sm.match
						.. "'",
					anchor
				)
...
local function assign_datetime_local(sm, match, year, month, day, hour, min, sec, msec)
	...
	if sm.options.parse_datetime_as == "string" then
		...
		sm.value = sm.options.type_conversion[sm.value_type]({
			year = year,
			month = month,
			day = day,
			hour = hour,
...
local function create_array(sm)
	...
	if sm.nested_arrays >= sm.options.max_nesting_depth then
		_error(
			sm,
			"Maximum nesting depth has exceeded "
				.. sm.options.max_nesting_depth
				.. ". If this larger nesting depth is required, feel free to set 'max_nesting_depth' in the parser options."
		)
	end
	...
	for i = 1, #sm.tables - 1 do
		if out_table[sm.tables[i]] == nil then
			...
			if (meta_out_table[sm.tables[i]]).type == "value" then
				_error(
					sm,
					"Cannot override previously definied value '"
						.. sm.tables[i]
						.. "' with new table definition: '"
						.. table.concat(sm.tables, ".")
						.. "'"
				)
			end
...
local function assign_value(sm)
	...
	for i = 1, #sm.keys - 1 do
		...
		elseif (meta_out_table[sm.keys[i]]).type == "value" then
			_error(
				sm,
				"Cannot override existing value '" .. sm.keys[i] .. "' in '" .. table.concat(sm.keys, ".") .. "'"
			)
		elseif (meta_out_table[sm.keys[i]]).type == "dictionary" then
			_error(
				sm,
				"Cannot override existing table '" .. sm.keys[i] .. "' in '" .. table.concat(sm.keys, ".") .. "'"
			)
		elseif (meta_out_table[sm.keys[i]]).type == "array" then
			_error(
				sm,
				"Cannot override existing array '" .. sm.keys[i] .. "' in '" .. table.concat(sm.keys, ".") .. "'"
			)
		end
...
local function error_invalid_state(sm)
	...
	local found = sm.input:sub(sm.i, sm.i)
	if found == "\r" or found == "\n" then
	...
	elseif sm.mode == "inside_array" then
		error_message = error_message
			.. "Inside an array, expected a ']', '}' (if inside inline table), ',', newline, or comment. Found: "
			.. found
	elseif sm.mode == "wait_for_newline" then
		error_message = error_message
			.. "Just assigned value or created table. Expected newline or comment before continuing."
	end
	...
	elseif restore.previous_state == "value" then
		...
		_error(
			...
			"close_inline_table should not be called from the previous state: "
				...
				.. ". Please submit an issue with your TOML file so we can look into the issue!"
		)
	end
end
...
local function generic_type_conversion(raw_value)
	return raw_value
end
...
function tinytoml.parse(filename, options)
	...
	if options then
		if options.max_nesting_depth ~= nil then
			assert(
				type(options.max_nesting_depth) == "number",
				"the tinytoml option 'max_nesting_depth' takes in a 'number'. You passed in the value '"
					.. tostring(options.max_nesting_depth)
					.. "' of type '"
					.. type(options.max_nesting_depth)
					.. "'"
			)
		end
		...
		if options.max_filesize ~= nil then
			assert(
				type(options.max_filesize) == "number",
				"the tinytoml option 'max_filesize' takes in a 'number'. You passed in the value '"
					.. tostring(options.max_filesize)
					.. "' of type '"
					.. type(options.max_filesize)
					.. "'"
			)
		end
		...
		if options.load_from_string ~= nil then
			assert(
				type(options.load_from_string) == "boolean",
				"the tinytoml option 'load_from_string' takes in a 'function'. You passed in the value '"
					.. tostring(options.load_from_string)
					.. "' of type '"
					.. type(options.load_from_string)
					.. "'"
			)
		end
		...
		if options.parse_datetime_as ~= nil then
			assert(
				type(options.parse_datetime_as) == "string",
				"the tinytoml option 'parse_datetime_as' takes in either the 'string' or 'table' (as type 'string'). You passed in the value '"
					.. tostring(options.parse_datetime_as)
					.. "' of type '"
					.. type(options.parse_datetime_as)
					.. "'"
			)
		end
		...
		if options.type_conversion ~= nil then
			assert(
				type(options.type_conversion) == "table",
				"the tinytoml option 'type_conversion' takes in a 'table'. You passed in the value '"
					.. tostring(options.type_conversion)
					.. "' of type '"
					.. type(options.type_conversion)
					.. "'"
			)
	...
	if options.load_from_string == false then
		...
		if file:seek("end") > options.max_filesize then
			error(
				"Filesize is larger than 100MB. If this is intentional, please set the 'max_filesize' (in bytes) in options"
			)
		end
		file:seek("set")
...
local function escape_string(str, multiline, is_key)
	if not is_key and #str >= 5 and str:find("%d%d") then
		...
		if validate_datetime(sm, sm.match) then
			if
				sm.value_type == "datetime"
				or sm.value_type == "datetime-local"
				or sm.value_type == "date-local"
				or sm.value_type == "time-local"
			then
				return sm.value
			end
		end
...
local function encode_element(element, allow_multiline_strings)
	if type(element) == "table" then
		...
		if is_array(element) then
			...
			if remove_trailing_comma then
				table.remove(encoded_string)
			end
